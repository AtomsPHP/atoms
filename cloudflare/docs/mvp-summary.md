...
| Pinned runtime artifact | *(not in the repo — staged into `worker/.php-wasm/`)* | WordPress Playground PHP 8.3 Asyncify build (64-bit ints). `npm ci` fetches it from `@php-wasm/web-8-3`; `scripts/prepare-runtime.mjs` stages and hash-verifies it in a gitignored directory. JSPI deliberately dropped — synchronous guest re-entry inside `transactionSync` is Asyncify-only. |
