...
|---|---|---|---|
| `PDO::exec()` | exec() return value for a CREATE TABLE, self-contained with a known preceding INSERT (M1 review F-6/F-12: title reworded) | supported |  |
...
| Member | Case | Status | Notes |
|---|---|---|---|
| `PDO::getAttribute()` | ATTR_AUTOCOMMIT (M1 review F-6: title reworded) | refused by both | Both sides refuse getAttribute(ATTR_AUTOCOMMIT): real pdo_sqlite answers SQLSTATE[IM001] (measured) and ours throws AtomsNotSupported. Refused_by_both, matching the driver's own posture exactly. |
| `PDO::getAttribute()` | ATTR_CASE | supported |  |
| `PDO::getAttribute()` | ATTR_CLIENT_VERSION (M1 review F-6: title reworded) | refused | getAttribute(ATTR_CLIENT_VERSION) is refused permanently for the same reason as ATTR_SERVER_VERSION: there is no client library on this side of the wire whose version would be truthful to report (design §3 F-22). |
...
| `PDO::getAttribute()` | ATTR_EMULATE_PREPARES (M1 review F-6: title reworded) | refused by both | Both sides refuse getAttribute(ATTR_EMULATE_PREPARES): real pdo_sqlite answers SQLSTATE[IM001] (measured) and ours throws AtomsNotSupported. Refused_by_both, matching the driver's own posture exactly. |
...
| `PDO::getAttribute()` | ATTR_SERVER_VERSION (M1 review F-6: title reworded) | refused | getAttribute(ATTR_SERVER_VERSION) is refused permanently: the guest's SQLite build and the Durable Object's SQLite build are two different binaries, so any single answer would misrepresent one of them (design §3 F-22). |
| `PDO::getAttribute()` | ATTR_TIMEOUT (M1 review F-6: title reworded) | refused by both | Both sides refuse getAttribute(ATTR_TIMEOUT): real pdo_sqlite itself answers SQLSTATE[IM001] (measured) and ours throws AtomsNotSupported — a genuine refused_by_both, not merely a gap on our side. No fix needed; the driver we claim to be doesn't answer this either. |
...
| `PDO::setAttribute()` | ATTR_DEFAULT_FETCH_MODE round-trip through getAttribute(), then restored (M1 review F-7) | supported |  |
...
| `PDO::lastInsertId()` | the PDO contract is a string, never an int — TYPE and CONTENT both compared (M1 review F-20) | supported |  |
| `PDO::lastInsertId()` | lastInsertId() across an intervening SELECT and a no-match UPDATE | supported |  |
| `PDO::lastInsertId()` | lastInsertId() with a sequence name argument (M1 review F-6: title reworded) | supported |  |
...
| Member | Case | Status | Notes |
|---|---|---|---|
| `value round-trip` | ±2^31 and ±(2^53-1) — inside the JS-safe-integer band, no precision loss possible (M1 review F-11: negative end added) | supported |  |
...
| Member | Case | Status | Notes |
|---|---|---|---|
| `PDO::errorCode()` | query() is a CONNECTION-level entry point — its own failure updates $pdo->errorCode() (M1 review F-8) | supported |  |
...
| `PDOStatement::columnCount()` | columnCount() over a result set with duplicate column names (M1 review F-6: title reworded) | supported |  |
| `PDOStatement::fetchColumn()` | fetchColumn(1) over duplicate column names | refused | fetchColumn(1) over duplicate column names now throws (M1 review F-1, FIX: fetchColumn() reached the wire directly and skipped the same duplicate-column guard every other FETCH_COLUMN path uses, silently answering an unrecoverable collapsed value under duplicates). Real pdo_sqlite answers the SECOND occurrence's original value from the true 4-value arity (measured); our wire's last-wins collapse has already discarded it before PHP could see it. Alias duplicate columns distinctly (see dup.aliased, which matches) instead. |
...
| `PDOStatement::debugDumpParams()` | debugDumpParams() with a bound named param and a bound positional param, exact captured-output byte comparison (design §3 F-10; M1 review F-4) | supported |  |
| `PDOStatement::debugDumpParams()` | debugDumpParams() with nothing bound, exact captured-output byte comparison (design §3 F-10) | supported |  |
| `PDOStatement::debugDumpParams()` | debugDumpParams() with two bound positional params, exact captured-output byte comparison (M1 review F-4) | supported |  |
| `PDOStatement::getAttribute()` | getAttribute(ATTR_CURSOR) on a prepared statement (M1 review F-6: title reworded) | refused by both | Both sides refuse PDOStatement::getAttribute() with the SAME SQLSTATE now (design §3 F-23: AtomsNotSupported's third constructor arg carries 'IM001', matching real pdo_sqlite's measured SQLSTATE[IM001] exactly — there is no driver-owned statement handle to read from on either side). A tightened implementation, not a loosened comparison — refused_by_both with a matching triple, not merely a matching family. |
