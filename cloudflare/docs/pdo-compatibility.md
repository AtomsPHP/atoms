...
| `PDO::getAttribute()` | ATTR_CLIENT_VERSION | refused | getAttribute(ATTR_CLIENT_VERSION) is refused permanently for the same reason as ATTR_SERVER_VERSION: there is no client library on this side of the wire whose version would be truthful to report. |
...
| `PDO::getAttribute()` | ATTR_SERVER_VERSION | refused | getAttribute(ATTR_SERVER_VERSION) is refused permanently: the guest's SQLite build and the Durable Object's SQLite build are two different binaries, so any single answer would misrepresent one of them. |
...
| `PDOStatement::execute()` | real PDO stringifies EVERYTHING passed to execute() — a PERMANENT pinned deviation | differs | execute([42, 42]) binds the array elements with their NATIVE PHP types on our side (int 42 stays INTEGER); real pdo_sqlite stringifies EVERY value passed to execute() regardless of its PHP type (measured). This is a DELIBERATE, PERMANENT pinned deviation, not a gap: matching real's stringify-everything behaviour would push wide integers through as TEXT literals and defeat inlineWideIntegers()'s int64 exactness. Use bindValue() with an explicit type when you need PDO's stringification. |
| `PDOStatement::execute()` | execute() called with an explicit empty array, over a previously bound value | refused | execute([]) is treated as an explicit (empty) replacement of any bound values, so the placeholder is left unsupplied. An 8.4-desktop measurement suggested the OPPOSITE fix (reuse bound values), but measured IN-GUEST — against this build's own real pdo_sqlite, the referee this milestone uses — execute([]) does NOT reuse a bound value either (it answers the unbound placeholder as NULL, its own SQLite default); the desktop measurement does not hold here. What differs is what workerd does with the resulting placeholder-count mismatch: ctx.storage.sql.exec() throws (sql_error) when given fewer bindings than '?' marks, where real SQLite's C API silently binds NULL for an unsupplied parameter. This is the SAME failure family as bind.named.missing: SQLite's own silent-NULL leniency here is a silent-wrong-answer risk, so refusing is not merely unfilled — pass execute(null) (no arguments) to keep bound values, which does match. |
| `PDOStatement::execute()` | a named and a positional placeholder bound in the SAME statement | refused | A statement binding both a named (:k) and a positional (?) placeholder throws SQLSTATE[HY093] on our side: NamedParams::rewrite() treats a bindings array containing ANY non-int key as entirely named, so the sole ? is never resolved and its bound value is reported as an unused named binding. Real pdo_sqlite executes this via SQLite's own shared ordinal sequence (named and positional placeholders share one ordinal, left to right in the SQL text) and answers a row. Implementing that ordinal model is deliberately out of scope; bind every placeholder in a statement the SAME way (all positional, or all named) to avoid this refusal. |
...
| `PDOStatement::bindValue()` | bindValue(0, ...) — position must be >= 1 | refused by both | bindValue(0, ...) throws a \ValueError on both sides ('must be greater than or equal to 1') — SQLite/PDO positions are 1-based, and this shim validates the same range real PDO does rather than silently accepting a bogus position that would only surface as a confusing failure later. |
| `PDOStatement::bindValue()` | the NEGATIVE end of the wide-int band (PHP_INT_MIN) bound as PARAM_INT and read back DIRECTLY | refused | Binding PHP_INT_MIN and reading it straight back (no CAST) hits the SAME int64_precision wall as bind.value.wide_int_param_int, just on the negative side of zero (runtime-spec.md Appendix item 1): workerd's SQL->JS conversion loses precision for any value in the ambiguous magnitude band regardless of sign. Bind the wide integer as a string instead (see bind.value.wide_int_param_str, which matches) and CAST(... AS TEXT) when reading it back. |
| `PDOStatement::bindValue()` | a >2^53-1 int bound as PARAM_INT and read back DIRECTLY hits the same int64 wall as a stored value | refused | Binding PHP_INT_MAX and reading it straight back (no CAST) hits the same int64_precision wall a stored wide integer does (runtime-spec.md Appendix item 1): workerd's SQL->JS conversion loses precision for any value in the ambiguous band regardless of how it entered SQLite. Bind the wide integer as a string instead (see bind.value.wide_int_param_str, which matches) and CAST(... AS TEXT) when reading it back. |
...
| `PDOStatement::bindColumn()` | bindColumn() with an unknown column name | refused by both | bindColumn('nope', ...) throws PDOException on both sides for an unknown column name: real pdo_sqlite (measured HY000, "Did not find column name 'nope' in the defined columns; it will not be bound") and ours (resolved against Branch A's `columns` list — HY000, same message shape). Both refuse; a genuine refused_by_both, not a gap — bindColumn() by name and by index against a known column IS filled and matches (see fetch.bound.default_type/fetch.bound.param_int). |
...
| `PDOStatement::fetchAll()` | FETCH_KEY_PAIR over a THREE-column result set | refused by both | fetchAll(FETCH_KEY_PAIR) over three columns now throws: real pdo_sqlite requires EXACTLY two columns and throws PDOException HY000 (measured); ours now checks `count($this->columns)` (Branch A) and throws the same family with a real errorInfo triple + getCode() as the SQLSTATE. A genuine refused_by_both, not a gap — FETCH_KEY_PAIR over exactly two columns IS filled and matches (see fetch.key_pair). |
| `PDOStatement::fetch()` | FETCH_LAZY | refused | fetch(FETCH_LAZY) throws unconditionally and permanently: real pdo_sqlite returns a PDORow, an internal class bound to a live statement that userland code cannot construct, so there is nothing on our side to reproduce it with. |
...
| `PDOStatement::fetch()` | FETCH_ORI_ABS on a forward-only cursor | refused | Same root cause and same permanent posture as fetch.ori_prior. |
| `PDOStatement::fetch()` | FETCH_ORI_FIRST on a forward-only cursor | refused | Same root cause and same permanent posture as fetch.ori_prior. |
| `PDOStatement::fetch()` | FETCH_ORI_LAST on a forward-only cursor | refused | Same root cause and same permanent posture as fetch.ori_prior. |
| `PDOStatement::fetch()` | FETCH_ORI_PRIOR on a forward-only cursor | refused | fetch() with any orientation other than FETCH_ORI_NEXT throws unconditionally and PERMANENTLY: real pdo_sqlite has no scrollable cursor and, measured, silently IGNORES the orientation and returns the NEXT row regardless — implementing scrolling over our buffer would either diverge from the driver or claim a capability the comparator itself does not have (the decisive measurement). Refusing is strictly more honest than the driver's own silent wrong-row behaviour. |
| `PDOStatement::fetch()` | FETCH_ORI_REL on a forward-only cursor | refused | Same root cause and same permanent posture as fetch.ori_prior. |
| `PDOStatement::setFetchMode()` | FETCH_CLASS with a class name argument | differs | setFetchMode(FETCH_CLASS, Row::class) then fetchAll() is FILLED and hydrates all three rows correctly. The one remaining difference is the SAME already-pinned root cause as value.float_integral: row 'b's REAL column (r=2.0) round-trips as PHP int(2) on our side, float(2) on real's, because our wire's encodeInt64Deep() cannot distinguish a whole-number REAL from an INTEGER once workerd hands both to JS as the same `number` type. Not a new limitation — an existing one now reachable through a newly-filled path. |
...
| `PDO::query()` | FETCH_KEY_PAIR as query()'s explicit per-statement mode | refused | Same corner as mode.stmt.default_key_pair, via PDO::query($sql, FETCH_KEY_PAIR): query() routes its fetch-mode arguments through setFetchMode(), so it inherits the same statement-level refusal. Measured: native pdo_sqlite answers the true pair. Follow-up alignment with mode.stmt.default_key_pair. |
...
| `PDOStatement::setAttribute()` | setAttribute(ATTR_CURSOR, CURSOR_FWDONLY) on a prepared statement | refused by both | Both sides refuse PDOStatement::setAttribute() with the SAME SQLSTATE now, for the same reason as getAttribute(): real answers SQLSTATE[IM001] (measured) and ours throws AtomsNotSupported constructed with 'IM001' — refused_by_both with a matching triple, not merely a matching family. |
| `PDO::sqliteCreateAggregate()` | same reasoning as sqlite_create_function | refused | Same permanent reasoning as stmt.sqlite_create_function — an aggregate callback is equally unreachable from the Durable Object's SQL engine. |
| `PDO::sqliteCreateCollation()` | same reasoning as sqlite_create_function | refused | Same permanent reasoning as stmt.sqlite_create_function — a collation callback is equally unreachable from the Durable Object's SQL engine. |
| `PDO::sqliteCreateFunction()` | a real sqlite_create_function extension the guest cannot register a callback with | refused | sqliteCreateFunction() throws unconditionally and PERMANENTLY: statements execute in the Durable Object, not in the guest, so a PHP callback registered here could never actually be invoked by the engine running the SQL. Real pdo_sqlite (same process as the SQL engine) supports it natively. |
