...
`conformance.mjs` runs 24 checks against a live worker URL:
...
15. **deadline overrun** — 15a uncaught: 504 `turn_deadline_exceeded`, residency stays healthy; 15b caught: an ordinary 200, and the latched budget stops a second `app()` from reaching the network
16. **`dispatch()` delivered, signed, `kind=job`** — args keyed by promoted constructor property name, delivered before the turn's own HTTP response
17. **`dispatch()` transaction semantics** — dropped on rollback, delivered on commit, delivered even when dispatched outside a transaction ahead of an uncaught throw
18. **WebSocket connect** — `onConnect` observed, the full query string delivered as `params`; bad upgrades (too many channels, a type with no handler) refused before any DO work
19. **echo round trip** — `onMessage` + `Connection::send()`, text and binary frames
20. **`broadcast()`** — reaches every connection on the channel and only that channel, exact wire shape, empty channel is not an error
...
own environment. Checks 18–24 have no such gate; they always run.
...
ATOMS_BASE_URL=http://localhost:8787 ATOMS_SKIP=12,21,24 node test/conformance.mjs
...
ATOMS_BASE_URL=http://localhost:8787 ATOMS_TURN_DEADLINE_MS=2000 node test/conformance.mjs
```
...
**Client tooling for the WebSocket checks (18–22): Node's built-in global
...
- `ATOMS_EVICTION_WAIT_MS` (default 12500) — milliseconds to wait for DO eviction in checks 12, 21 and 24
