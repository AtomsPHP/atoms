...
| ATOMS-E032 | AtomJob constructor signature mismatch | error | build | AtomJob constructor parameters are the dispatch contract — keep the promoted properties and the dispatch site in sync. With $this->dispatchJob() the array keys are the constructor parameter names. |
...
| ATOMS-E061 | Turn deadline exceeded | error | runtime | Turns must stay short. Move slow work to an AtomJob via $this->dispatchJob(), and check for accidental blocking I/O. |
...
| ATOMS-E082 | app() called inside a database transaction | error | runtime | Read what you need from the monolith before opening the transaction, or close the transaction and re-open it after the call. $this->dispatchJob() is allowed inside a transaction — it is queued on commit and dropped on rollback. |
...
| ATOMS-E084 | Dispatched job cannot be encoded from its promoted properties | error | runtime | An AtomJob's constructor parameters are its dispatch contract and must be promoted public properties. Declare public function __construct(public readonly int $orderId) — non-promoted state cannot cross the boundary. |
...
| ATOMS-E101 | Sleep called inside Atom code | error | static-analysis | Delete the sleep. To act later, schedule a timer with $this->timers()->schedule($name, $at) and do the work in onTimer(); to hand work to your app, use $this->dispatchJob(). Nothing in an Atom may wait on wall-clock time. |
| ATOMS-E102 | Elapsed-time wait loop inside Atom code | error | static-analysis | Rewrite the loop so it terminates on data rather than on elapsed time, or move the wait out of the Atom: $this->timers()->schedule() delivers onTimer() in a later turn, and $this->dispatchJob() runs the work in your app. |
...
| ATOMS-E104 | AtomJob constructed inside Atom code | error | build | Dispatch it by name instead — $this->dispatchJob(TheJob::class, ['param' => $value, ...]) — passing the constructor arguments by name. Naming a class does not require it to ship, and your app reconstructs the real job from those arguments. |
