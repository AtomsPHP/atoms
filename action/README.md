...
| `ATOMS-E110` | The script uploaded, but its routing did not attach | The environment is live on new code behind its old routing. Either a route pattern belongs to another Worker (Cloudflare refuses it rather than moving it), or the token lacks **Zone → Workers Routes:Edit** and **Zone → Zone:Read** on that zone. Fix and re-run; the deploy is safe to repeat. |
