...
Every user-facing Atoms failure has a stable code and a concrete fix. Search this page for the code from your exception or command output. This page is generated from `packages/core/resources/errors.json` so it cannot drift from the shipped catalog.
...
Rename one of the two declarations, or move it to its own namespace. A bundle carries both files, so the guest would fatal at class-declaration time on whichever one loads second.
...
## ATOMS-E020: Boundary type outside the serialization algebra
...
The Atom stays deactivated. Fix the migration by appending a corrective one — never edit the failed file — and redeploy.
...
The SDK retries automatically where safe. Persistent refusals mean the fleet is at capacity — check the dashboard.
...
## ATOMS-E067: WebSocket ticket acquisition failed
...
Run `npm install` in the Worker directory so node_modules/.bin/wrangler exists, or set ATOMS_WRANGLER_BIN to an absolute path. Atoms never downloads Wrangler for you.
...
Point "worker_dir" in atoms.json at a checkout of the Atoms Cloudflare Worker (cloudflare/worker), and run `npm ci` inside it.
...
Pass the constructor arguments to $this->dispatch() as a map keyed by parameter name — ['orderId' => $id], not [$id] — and make sure every value is a serialization-algebra type. An AtomJob's constructor parameters are its dispatch contract and must be promoted public properties, so your app can rebuild it from that map.
...
{symbol}() is called in {class}, but the Atoms runtime's clock does not advance inside a turn — the call blocks until the turn deadline kills it.
...
ATOMS_SHARED_SECRET is missing, or does not decode to exactly 32 bytes of base64.

**Fix**

Generate one with `openssl rand -base64 32` and configure the identical value on the app and the Worker (`wrangler secret put ATOMS_SHARED_SECRET`). The secret itself is never transmitted: clients authenticate with the bearer derived from it (`atoms token` prints it). ATOMS_SHARED_SECRET_PREVIOUS, when set for rotation, must satisfy the same format.
