...
The Atom stays deactivated, and the failed migration is rolled back. Check the reported cause and resolve it before retrying activation. Later migrations cannot run until this one succeeds. Keep shipped migrations unchanged; a corrective migration can repair a schema change that applied successfully, but cannot bypass a failing migration.
...
This code appears in older logs. Applications now issue tickets locally with Atoms\Client\Tickets\TicketIssuer. Invalid claims report ATOMS-E068; a missing or malformed shared secret reports ATOMS-E105.
...
Generate one value with `openssl rand -base64 32`, save it in your secret manager, and configure the identical value on the application and Worker (`atoms shared-secret:set` reads it from stdin). The secret is not a bearer token: use `atoms token` to derive a token for requests. ATOMS_SHARED_SECRET_PREVIOUS uses the same format.
